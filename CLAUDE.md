...
3. **Kod liczy, szablony objaśniają.** Opis wyniku (`quantlab narrate`, panel „Narrative") składa `TemplateNarrator` z liczb w magazynie wyników (`q13`), bez modelu językowego: płatne API łamie zasadę darmowych danych, a lokalny model jest za wolny na tej maszynie. Gdyby LLM kiedyś doszedł, to jako druga implementacja `Narrator`: żadnych obliczeń finansowych w modelu, a każda liczba w tekście sprawdzana z faktami.
4. **Bez rekomendacji inwestycyjnych.** Raporty opisują wyniki historyczne, ekspozycje i ryzyko — nie mówią „kup / sprzedaj / zwiększ".
5. **Dane cenowe: tylko darmowe źródła, repozytorium prywatne** ([ADR-0009](docs/adr/0009-private-repo-free-tiingo-plan.md)). Żadnych płatnych planów. Darmowe plany (Tiingo Starter) pozwalają tylko na użytek własny, więc wyniki z cenami rynkowymi się nie publikuje. Surowe dane nie trafiają do repo (gitignored), tylko syntetyczne fixtures do testów. Patrz [docs/DATA-SOURCES.md](docs/DATA-SOURCES.md).
