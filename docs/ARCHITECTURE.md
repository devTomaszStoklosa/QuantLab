...
7. Tylko darmowe źródła danych (Binance, plan Starter Tiingo), repozytorium prywatne ze względu na licencję użytku własnego ([ADR-0009](adr/0009-private-repo-free-tiingo-plan.md)).
8. `core` nie zależy od żadnej warstwy domenowej; warstwy domenowe zależą od `core`, nie od siebie nawzajem poza jawnymi punktami integracji (`reporting` czyta wyniki `backtest`; `validation` i `risk` czytają wyniki `backtest`).
...
8. **Trade & attribution analysis** (`quantlab.attribution`) — rejestr `Trade`, cięcia P&L: reżim przy wejściu, czas trzymania, klasa aktywów i instrument (`q12`).
9. **Reporting layer** (`quantlab.reporting`) — tear-sheet z metrykami liczonymi własnym kodem.
10. **Narrative** (`quantlab.reporting.narrative`, `q13`) — `TemplateNarrator` (implementacja `Narrator`) składa po polsku sekcje wpisu do dziennika z faktów odczytanych z magazynu wyników i rejestru (`results_store.stored_facts`). Formatuje liczby i porównuje je tylko z progami definicji, niczego nie liczy. `quantlab narrate` daje szkic wpisu, a odświeżenie rejestru zapisuje akapity w tabeli `narrative`, którą pokazuje aplikacja.
11. **Orchestration/CLI** (`quantlab.cli`) — `quantlab run [HIPOTEZA]`, `quantlab open-holdout [HIPOTEZA]`, `quantlab compare-engines [HIPOTEZA]` (opisowe porównanie silników na okresie treningowym) i `quantlab trials [HIPOTEZA]` (wszystkie próby na tych samych danych z PSR, DSR i PBO; próby liczone z historii gita `config/holdout/`, `quantlab.research.trials`), `quantlab registry` (rejestr hipotez w magazynie wyników, bez pobierania danych), `quantlab narrate [HIPOTEZA]` (`q13`: szkic wpisu do dziennika z liczb w magazynie wyników i zapisu otwarcia holdoutu, bez pobierania danych), `quantlab plan [--run]` (`q10`, `quantlab.research.plan`: stan każdego pozostałego kroku lokalnego — sprawdzenia, uniwersa, przebiegi, otwarcia holdoutów, commity, dziennik — liczony z repozytorium i magazynu; `--run` wykonuje sprawdzenia i kroki automatyczne, nigdy ręczne) i `quantlab check-source SOURCE` (czy Binance / Tiingo odpowiada). `quantlab run` zapisuje dowody przebiegu głównego do magazynu wyników `results/` (`quantlab.reporting.results_store`, [ADR-0008](adr/0008-results-store-parquet-duckdb.md)); `run` i `open-holdout` odświeżają przy tym rejestr. Parametry hipotezy pochodzą wyłącznie z jej zacommitowanej definicji `config/holdout/<id>.yaml` (przebieg bez commitu jest odrzucany przed pobraniem danych); stałe w CLI to tylko metodologia wspólna dla wszystkich hipotez (seed, liczba permutacji, reżimy, scenariusze stress testu).
12. **Presentation** (poza pakietem Python, rozszerzenie `q7`) — ASP.NET Core Web API + React, czyta wyłącznie magazyn wyników: tabele Parquet w `results/` (schemat w [specs/q7-dotnet-react-presentation/02-spec.md](specs/q7-dotnet-react-presentation/02-spec.md)). Syntetyczny magazyn do testów i trybu demo: `presentation/fixtures/results/`, generowany pełnym pipeline'em (`tests/test_results_fixture.py`).
...
- `DataProvider` (interfejs): `fetch(instrument, start, end) -> list[PriceBar]`. Implementacje: `BinanceProvider` ([ADR-0007](adr/0007-binance-not-stooq-for-first-adapter.md) — Stooq odrzucony, blokuje dostęp programistyczny) i `TiingoProvider` (akcje i ETF-y USA, `q5`: surowe ceny, dywidendy i splity z wierszy cen, delisting z ostatniego dnia notowań w metadanych, bez zwrotu z delistingu; odstęp żądań i licznik tickerów w limitach darmowego planu, [ADR-0009](adr/0009-private-repo-free-tiingo-plan.md)). Runner bierze dostawcę z rejestru nazw (`cli._PROVIDERS`) po polu `source` pliku uniwersum, bez `if` po klasie aktywów.
...
- Zmienność ex-ante instrumentu (`risk.volatility`, `q11`): `RollingVolatility` (odchylenie z okna) i `EwmaVolatility` (wykładniczo ważona, środek masy c dni); oba czytają dokładnie `window_days` zwrotów do dnia decyzji włącznie.
