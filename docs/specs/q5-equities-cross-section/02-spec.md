...
- **Pomiar w skali `xsmom_v1` (po X8, 2026-09-24):** 900 spółek × 16 lat (2003-10 → 2019-12, rotacja składu), momentum 12-1, decyle long-short, wszystkie modele kosztów, 10 000 tasowań dni i 10 000 losowych portfeli: z barami jako modelami pydantic 339 s i **6,0 GB** szczytowo — ponad możliwości maszyny deweloperskiej (8 GB z systemem). Po zamianie `PriceBar` na dataclass ze slotami (298 zamiast 1 455 bajtów na bar, ten sam interfejs) 248 s i **1,7 GB**; analiza wrażliwości na zwrot z delistingu korzysta z tych samych barów zamiast pobierać je drugi raz.
- Kalibracja testu losowych portfeli (X7b): na 60 syntetycznych rynkach bez przewagi (błądzenie losowe, 40 spółek, 4 lata, momentum 12-1, 400 portfeli) średnie p = 0.455, p < 0.1 w 8.3% przypadków, rozkład p bliski jednostajnemu — test nie zawyża istotności. Druga seria po X8: 30 rynków bez przewagi z rotacją składu (120 spółek, 16 lat, 1 000 portfeli): p < 0.1 w 10% przypadków, średnie p = 0.415.
